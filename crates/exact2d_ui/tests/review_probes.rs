//! Review probes for the UI application layer — integration of snapping, tools,
//! and undo through `AppState`.
...
/// With snapping on, clicking *near* an existing endpoint must snap the new
/// geometry exactly to that endpoint.
#[test]
fn line_tool_snaps_to_existing_endpoint() {
    ...
    // Click slightly off the endpoint (0,0): screen pos for world (0.05, 0.05).
    ...
    // A new segment should connect EXACTLY (0,0)→(10,0) via snapping.
...
/// Selecting via click then ERASE must remove exactly that entity.
#[test]
fn click_select_then_erase() {
    let mut a = app_with_line();
    a.snap_on = false;
    // Default tool is Select; click on the midpoint of the line (5,0).
    ...
    a.run_command("ERASE");
    assert_eq!(a.document.len(), 1); // origin remains
}

/// A full draw → undo → redo cycle through the app must restore geometry exactly.
#[test]
fn draw_undo_redo_cycle() {
    ...
    assert_eq!(a.document.len(), 2); // line + origin

    a.undo();
    assert_eq!(a.document.len(), 1); // origin
    a.redo();
    assert_eq!(a.document.len(), 2); // line + origin
...
/// COPY through the app on a selected entity must keep the original and add a
/// translated copy.
#[test]
fn copy_command_full_flow() {
    ...
    a.selection = vec![id];
    a.run_command("COPY");
    // base (0,0) → dest (0,5): copy translated up by 5.
